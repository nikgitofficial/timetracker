...
export default function TimeClockPage() {
  ...
  // ✅ FIX: Added entryId to actionModal state for exact DB match
  const [actionModal, setActionModal] = useState<{ action: Action; image: string; message: string; entryId?: string } | null>(null);
...
=======
          videoRef.current?.play();
          setCameraReady(true);
          // ✅ FIX: startCountdown now waits for real video dimensions
          startCountdown();
>>>>>>> c0060f6 (fixing selfie route bugs issue)
        };
      }
  ...
  // ✅ FIX: startCountdown waits until video has real dimensions before counting down
...
=======
    const waitForVideo = () => {
      if (videoRef.current && videoRef.current.videoWidth > 0) {
        // Video is truly ready — start countdown
        setCountdown(3);
        let count = 3;
        countdownRef.current = setInterval(() => {
          count -= 1;
  ...
  // ✅ FIX: capturePhoto guards against zero dimensions and retries if needed
  const capturePhoto = () => {
    if (!videoRef.current || !canvasRef.current) return;
>>>>>>> c0060f6 (fixing selfie route bugs issue)
    const video = videoRef.current;

    // Guard: make sure video has actual dimensions
    if (!video.videoWidth || !video.videoHeight) {
      console.warn("Video not ready yet, retrying in 300ms...");
...
=======
  // ✅ FIX: uploadPhoto with retry logic + entryId for exact DB match
  const uploadPhoto = async (dataUrl: string, retryCount = 0) => {
...
=======
      // ✅ Pass entryId for exact DB match
      if (actionModal.entryId) {
        formData.append("entryId", actionModal.entryId);
      }
...
=======
      ...
      console.error(`Selfie upload error (attempt ${retryCount + 1}):`, err);

      // ✅ Retry up to MAX_RETRIES times
      if (retryCount < MAX_RETRIES) {
        console.log(`Retrying upload in ${RETRY_DELAY}ms... (${retryCount + 1}/${MAX_RETRIES})`);
        setTimeout(() => {
          uploadPhoto(dataUrl, retryCount + 1);
        }, RETRY_DELAY);
        return;
      } else {
        // All retries exhausted — show error but don't block user
        console.error("Selfie upload failed after all retries");
        setPhotoUploaded(false);
        setMessage({ text: "⚠️ Selfie upload failed but your punch was recorded!", type: "error" });
      }
    } finally {
      // Only clear uploading state if not retrying
      if (retryCount === 0) {
  ...
  const handleAction = async (action: Action) => {
    ...
    try {
      ...
      if (!res.ok) { setMessage({ text: data.error || "Action failed", type: "error" }); }
        ...
        // ✅ FIX: Pass entryId so selfie API finds exact entry
        setActionModal({ action, image, message: msg, entryId: data.entry?._id });
      }
